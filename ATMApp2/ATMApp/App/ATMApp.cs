...
namespace ATMApp
{
    public class ATMApp : IUserLogin, IUserAccountActions, ITransaction
    {
        ...
        public void InitializeData()
        {
            ...
            {
                new UserAccount{Id=1, FullName = "Shruthi", AccountNumber=123456,CardNumber =321321, CardPin=123123,AccountBalance=50000.00m,IsLocked=false},
                new UserAccount{Id=2, FullName = "Lirin Stalin", AccountNumber=456789,CardNumber =654654, CardPin=456456,AccountBalance=4000.00m,IsLocked=false},
                new UserAccount{Id=3, FullName = "Bhavani", AccountNumber=123555,CardNumber =987987, CardPin=789789,AccountBalance=2000.00m,IsLocked=true},
            };
        ...
        public void checkAdmin(bool isAdminLoginCorrect)
        {
            string AdminId;
            int AdminPassword;
            AppScreen.AdminWelcome(AdminId, AdminPassword);
            if(AdminId="Bank@123"&&AdminPassword="657483")
            {
                isAdminLoginCorrect = true;
            }
            else
            {
                Console.WriteLine("Sorry the id or password is incorrect")
                Mainmenuprocess();
            }
        }
        public void LoadCash()
        {
            bool isAdminLoginCorrect = false
            checkAdmin(isAdminLoginCorrect);
            while(isAdminLoginCorrect==true)
            {
                Console.WriteLine("Please enter the denominations")
                int twothou = Validator.Convert<int>("2000 X.");
                int fivehun = Validator.Convert<int>("500 X.");
                int twohun = Validator.Convert<int>("200 X.");
                int onehun = Validator.Convert<int>("100 X.");
                long total_amt = (2000 * twothou) + (500 * fivehun) + (200 * twohun) + (100 * onehun);
                Console.WriteLine("The total amount of cash loaded is :",total_amt)
        ...
        public void CustomerDetails()
        {
            ...
            while (isAdminLoginCorrect == true)
            {
                var table = new ConsoleTable("Account Number", "Account Holder", "Pin Number", "Account Balance");
                foreach (var user in userAccountList)
                {
                    table.AddRow(user.AccountNumber, user.FullName, user.CardPin, user.AccountBalance);
                }
